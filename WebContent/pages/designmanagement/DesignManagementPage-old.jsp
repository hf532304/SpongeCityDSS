...
<div style="font-size:10px;" align="center">
	...
	<table border="1" cellspacing="0" cellpadding="2">
		<tr><td></td><td>项目编号</td><td>项目名称</td><td>项目描述</td><td>方案编号</td><td>方案名称</td><td>方案状态</td><td>创建者</td><td>创建时间</td></tr>
	<%for (int i=0; i<myDesignData.length; i++){ %>
		<tr><td><input type="radio" name="DefaultDesign" value="<%=myDesignData[i].projectId%>;<%=myDesignData[i].designId%>" <%=myDesignData[i].isDefault()%>/></td><td><%=myDesignData[i].projectId %></td><td><%=myDesignData[i].projectName%></td><td width="40"><%=myDesignData[i].projectDesc%></td><td><%=myDesignData[i].designId %></td><td><%=myDesignData[i].designName%></td><td><%=myDesignData[i].designStatus%></td><td><%=myDesignData[i].creator%></td><td><%=myDesignData[i].creationTime%></td></tr>
	  <%}%>
	</table>
	<br/>
	...
	<table> 
		...
		<input type="submit" onclick="document.getElementById('Name').value='ProjectDeleteSubmitPage'" value="删除项目"></td>
	</tr></table><br/>
	<table><tr>
		...
		<td><img src="images/clear.gif" width="10" height="1" border="0" alt="">
		<input type="submit" onclick="document.getElementById('Name').value='DesignDeleteSubmitPage'" value="删除方案"></td>
		</tr>
	</table>
	<%} else {%>	
		<tr><td align="center">尚无任何项目计划</td></tr>
	</table>
	<br/><br/>
	<table> 
		<tr><td><input type="submit" onclick="document.getElementById('Name').value='ProjectNewPage'" value="新建"></td></tr>
	</table>
  <%}%>
	</div>
</form>
